...
parser.add_argument('--json', help='JSON description of P4 program [deprecated]',
                    action="store", required=False)
parser.add_argument('--debug-file', help='File containing list of commands for the debugger',
                    action="store", required=False)
...
class DebuggerAPI(cmd.Cmd):
    ...
    def load_dbg_breakpoints(self, file):
        if(file is None): return
        commands = {"continue", "set_wp", "break", "show_wps", "show_bps"}
        commands_short = {"c", "w", "b"}
        with open(file) as fp:
            for line in fp:
                if not line.strip(): continue
                line_split = line.split()
                my_cmd = line_split.pop(0).strip()
                args = ' '.join(line_split)
                print "Set p4dbg command: %s %s" % (my_cmd, args)
                if((my_cmd in commands) or (my_cmd in commands_short)): # IFs should be mutually exclusive
                    if ((my_cmd == "continue")  or (my_cmd == "c"))  : self.do_continue(args)    ; continue
                    if ((my_cmd == "set_wp")    or (my_cmd == "w"))  : self.do_set_wp(args)      ; continue
                    if ((my_cmd == "break")     or (my_cmd == "b"))  : self.do_break(args)       ; continue
                    if ((my_cmd == "show_wps"))                      : self.do_show_wps(args)    ; continue
                    if ((my_cmd == "show_bps"))                      : self.do_show_bps(args)    ; continue
                    raise UIn_Error("Unknown p4dbg command")
...
def main():
    ...
    try:
        c.attach()
        c.load_dbg_breakpoints(args.debug_file)
        c.cmdloop()
    except KeyboardInterrupt:
        if c.attached:
            c.say_bye()
