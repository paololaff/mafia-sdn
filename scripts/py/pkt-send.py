...
from scapy.all import Ether, IP, UDP, TCP, ICMP, sendp, get_if_hwaddr, get_if_list, Raw
import sys
import argparse
...
def send_random_traffic(dst, proto, n, connections, port, random_port):
    dst_mac = None
    dst_ip = None
    ...
    # pkt = Ether(dst=dst_mac,src=src_mac)/IP(dst=dst_ip,src=src_ip)/TCP(sport=src_port,dport=dst_port,flags="S")
    ...
    for c in range(0, connections):
        ...
        for i in range(n - 1):
            ...
            if i == 1:
                ...
                pkt = Ether(dst=dst_mac,src=src_mac)/IP(dst=dst_ip,src=src_ip)/TCP(sport=src_port,dport=dst_port,flags=0)/Raw(load=data)
                sendp(pkt, iface = "eth0", verbose=False)
            total_pkts += 1
            # time.sleep(1)
        # print "Sending FIN"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--port',          '-p', type=int, help='TCP/UDP port to use',                    required=False, default=80)
    parser.add_argument('--proto',          '-r', type=int, help='Protocol to use',                    required=False, default=80)
    ...
    send_random_traffic(args.dst, args.proto, n, args.n_connections, args.port, args.random_port)
